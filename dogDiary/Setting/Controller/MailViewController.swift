...
//  dogDiary
//
//  Created by najin on 2020/12/14.
//
...
class MailViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func sendSMSButtonClick(_ sender: UIButton) {
        ...
        let URL = Common().baseURL+"/diary/member/mail-auth-insert"
        ...
        alamo.responseDecodable(of: Auth.self) { (response) in
            ...
            if auth.id == 0 {
                //이미 등록된 이메일인 경우
                self.mailCheckLabel.isHidden = false
                self.sendMailButton.setTitle("인증번호 전송", for: .normal)
    ...
    @IBAction func nextButtonClick(_ sender: UIButton) {
        if authNumberTextField.text == String(authNumber) {
            ...
            if timeLeft == 0 {
                ...
                mail = self.mailTextField.text!
                let alamo = AF.request(URL, method: .put, parameters: ["type": "mail", "mail": mail], encoder: JSONParameterEncoder.default).validate(statusCode: 200..<300)
                
                alamo.response { (response) in
                    switch response.result {
                    case .success(_):
                        MemberVO.shared.mail = self.mail
